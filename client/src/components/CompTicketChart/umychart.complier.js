...
var g_JSComplierResource = {
  ...
  GetDrawIcon: function(id) {
    ...
    if (g_JSComplierResource.CustomDrawIcon.Data.has(id)) {
      ...
      //SVG图标
      else icon = { Symbol: iconfont.Text, Color: iconfont.Color, Family: iconfont.Family, IconFont: true, ID: id };
      return icon;
    }
  ...
  GetDrawTextIcon: function(id) {
    //图标对应的字符代码
    let mapIcon = new Map([
      [1, { Symbol: '↑', Color: 'rgb(238,44,44)' }],
      [2, { Symbol: '↓', Color: 'rgb(0,139,69)' }],
      [3, { Symbol: '😧' }],
      [4, { Symbol: '😨' }],
      [5, { Symbol: '😁' }],
      [6, { Symbol: '😱' }],
      [7, { Symbol: 'B', Color: 'rgb(238,44,44)' }],
      [8, { Symbol: 'S', Color: 'rgb(0,139,69)' }],
      [9, { Symbol: '💰' }],
      [10, { Symbol: '📪' }],
      [11, { Symbol: '👆' }],
      [12, { Symbol: '👇' }],
      [13, { Symbol: 'B', Color: 'rgb(178,34,34)' }],
      [14, { Symbol: 'S', Color: 'rgb(0,139,69)' }],
      [36, { Symbol: 'Χ', Color: 'rgb(238,44,44)' }],
      [37, { Symbol: 'X', Color: 'rgb(0,139,69)' }],
      [38, { Symbol: '▲', Color: 'rgb(238,44,44)' }],
      [39, { Symbol: '▼', Color: 'rgb(0,139,69)' }],
      [40, { Symbol: '◉', Color: 'rgb(238,44,44)' }],
      [41, { Symbol: '◈', Color: 'rgb(238,44,44)' }],
      [42, { Symbol: '📌' }],
      [43, { Symbol: '💎' }],
      [44, { Symbol: '🥇' }],
      [45, { Symbol: '🥈' }],
...
JSComplierHelper.GetPeriodInfo = function(obj) {
  ...
  if (obj.Name) {
    for (var i in PERIOD_LIST) {
      if (obj.Name && PERIOD_LIST[i].Name == obj.Name) return PERIOD_LIST[i];
    }
  ...
  if (IFrameSplitOperator.IsNumber(obj.PeriodID)) {
    for (var i in PERIOD_LIST) {
      if (PERIOD_LIST[i].Period == obj.PeriodID) return PERIOD_LIST[i];
    }
...
function Scanner(code, ErrorHandler) {
  ...
  this.Lex = function() {
    if (this.IsEOF())
      return { Type: 2 /*EOF*/, Value: '', LineNumber: this.LineNumber, LineStart: this.LineStart, Start: this.Index, End: this.Index };
...
function Node(ErrorHandler) {
  ...
  this.VerifySymbolVariable = function(varName, token) {
    ...
    if (varName.indexOf('#') > 0) {
      ...
      if (aryWrods.length != 2) {
        ...
        var item = {
          ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_SYMBOL_PERIOD_DATA,
          VarName: varName,
          ValueName: aryWrods[0],
          PeriodName: aryWrods[1],
        };
        if (token) item.Token = { Index: token.Start, Line: token.LineNumber };
    ...
    if (g_JSComplierResource.IsCustomVariant(varName)) {
      //自定义函数
  ...
  this.VerifySymbolFunction = function(callee, args, token) {
    ...
    if (callee.Name == 'UPCOUNT' || callee.Name == 'DOWNCOUNT') {
      //上涨下跌个数
...
function JSAlgorithm(errorHandler, symbolData) {
  ...
  this.REFX = function(data, n) {
    ...
    if (typeof n == 'number') {
      ...
      for (let i = 0; i < n; ++i) result.push(lastData);
    } //n 为数组的情况
    else {
  ...
  this.DMA = function(data, data2) {
    ...
    for (index = index + 1; index < data.length; ++index) {
      if (data[index] == null || data2[index] == null) result[index] = null;
      else {
        if (data2[index] < 1) result[index] = data2[index] * data[index] + (1 - data2[index]) * result[index - 1];
    ...
    if (Array.isArray(dayCount)) {
      for (i = 0; i < dayCount.length && i < data.length; ++i) {
        ...
        for (var j = start, k = 1; j < period; ++j, ++k) {
          index = i - (period - j - 1);
          value = data[index];
          if (this.IsNumber(value)) preValue = value;
          else value = preValue;
  ...
  this.MEMA = function(data, dayCount) {
    ...
    for (; i < data.length; ++i) {
      if (this.IsNumber(result[i - 1]) && this.IsNumber(data[i])) result[i] = (data[i] + result[i - 1] * (dayCount - 1)) / dayCount;
      else if (i - 1 > -1 && this.IsNumber(result[i - 1])) result[i] = result[i - 1];
      else result[i] = null;
    }
    return result;
  };
  ...
  this.EXPMA = function(data, dayCount) {
    ...
    for (i = i + 1; i < data.length; ++i) {
      if (result[i - 1] != null && data[i] != null) result[i] = (2 * data[i] + (dayCount - 1) * result[i - 1]) / (dayCount + 1);
      else if (result[i - 1] != null) result[i] = result[i - 1];
    }
  ...
  this.EXPMEMA = function(data, dayCount) {
    ...
    for (var i = 0; index < data.length && i < dayCount; ++i, ++index) {
      if (data[index] && !isNaN(data[index])) sum += data[index];
      else sum += data[index - 1];
    }
    ...
    for (; index < data.length; ++index) {
      ...
      else if (result[index - 1] != null) result[index] = result[index - 1];
    }
  ...
  this.SUMBARS = function(data, data2) {
    ...
    for (i = data.length - 1; i >= start; --i) {
      var total = 0;
      for (j = i, total = 0; j >= start && total < data2[i]; --j) total += data[j];
  ...
  this.REVERSE = function(data) {
    ...
    for (; i < data.length; ++i) {
      if (!this.IsNumber(data[i])) result[i] = null;
      else result[i] = 0 - data[i];
    }
    return result;
  };
  ...
  this.DEVSQ = function(data, n) {
    ...
    if (Array.isArray(data)) {
      ...
      for (E = 0; i < datanum && j < num; ++i, ++j) E += data[i] / num;
      if (j == num) {
        DEV = 0;
        for (i--; k < num; k++) DEV += (data[i - k] - E) * (data[i - k] - E);
        result[i] = DEV;
        i++;
      }
      for (; i < datanum; ++i) {
        E += (data[i] - data[i - num]) / num;
        for (DEV = 0, k = 0; k < num; ++k) DEV += (data[i - k] - E) * (data[i - k] - E);
        result[i] = DEV;
      }
    }
    ...
    if (this.IsNumber(n) && Array.isArray(data)) {
      ...
      for (; j < data.length; ++j) {
        if (this.IsNumber(result[j - 1])) {
          ...
          if (!(Math.abs(data[j]) < 0.000001)) result[j] = 0;
        }
      }
    }
  ...
  this.GetFirstVaildIndex = function(data) {
    for (var i = 0; i < data.length; ++i) {
      if (this.IsNumber(data[i])) return i;
    }
  ...
  this.ZIG_Calculate = function(data, dRate) {
      ...
      if (data[i] >= data[i - 1] && data[i] >= data[i + 1]) {
        if (lState < 0) {
          if ((data[i] - data[-lState]) * 100 < dRate * data[-lState]) continue;
            ...
            dest[j--] = data[-lState];
            for (; j >= lLastPos; j--) dest[j] = data[-lState] + (-lState - j) * dif;
            lLastPos = -lState;
            lState = i;
          }
        ...
        if (lState > 0) {
          if ((data[lState] - data[i]) * 100 < dRate * data[lState]) continue;
            ...
            for (; j <= lState; ++j) dest[j] = data[lLastPos] + (j - lLastPos) * dif;
            lLastPos = lState;
            lState = -i;
          }
        } else if (data[i] < data[-lState]) lState = -i;
    ...
    if ((lState = Math.abs(lState)) < nDataCount - 1) {
      if (data[nDataCount - 1] >= data[lState]) {
        ...
        dest[j++] = data[lState];
        for (; j < nDataCount; ++j) dest[j] = data[lState] + (j - lState) * dif;
      } else {
        dif = (data[lState] - data[(j = nDataCount - 1)]) / (nDataCount - lState);
        dest[j--] = data[nDataCount - 1];
        for (; j >= lState; j--) dest[j] = data[nDataCount - 1] + (nDataCount - j) * dif;
      }
    }
  ...
  this.COST = function(data, node) {
    ...
    var dMaxPrice = kData[0].High,
      dMinPrice = kData[0].Low;
    ...
    for (var i = 0; i < kData.length; ++i) {
      ...
      if (aryCapital[i] > 1) {
        ...
        for (var j = 0; j < lSpeed; j++) aryVolPrice[j] *= 1 - dHSL;
  ...
  this.LWINNER = function(n, data, node) {
    ...
    for (var i = startDay - 1, j = 0, k = 0; i < kData.length; ++i) {
      ...
      var dMaxPrice = kData[index].High,
        dMinPrice = kData[index].Low;
  ...
  this.UPNDAY = function(data, n) {
    ...
    for (let i = 0; i < data.length; ++i) {
      result[i] = 0;
      if (i - 1 < 0) continue;
      if (!this.IsNumber(data[i]) || !this.IsNumber(data[i - 1])) {
        //无效数都不算连涨
        days = 0;
        continue;
      }
  ...
  this.BETWEEN = function(condition, data, data2) {
    ...
    for (var i in condition) {
      ...
      if (left > right) {
        if (item >= right && item <= left) result[i] = 1;
      } else {
        if (item <= right && item >= left) result[i] = 1;
      }
    }
  ...
  this.STRCAT = function(str1, str2) {
    var result = [];
    if (IFrameSplitOperator.IsString(str1) && IFrameSplitOperator.IsString(str2)) result = str1 + str2;
    return result;
  };
  ...
  this.VARCAT = function(data, data2) {
    ...
    if (Array.isArray(data) && Array.isArray(data2)) {
      ...
      for (var i = 0; i < nCount; ++i) {
        ...
        if (i < data.length) {
          var item = data[i];
          if (IFrameSplitOperator.IsString(item)) strValue += item;
        }
  ...
  this.STRSPACE = function(data) {
    ...
    if (Array.isArray(data)) {
      for (var i = 0; i < data.length; ++i) {
        result[i] = null;
        var item = data[i];
        if (IFrameSplitOperator.IsString(item)) result[i] = item + ' ';
      }
    } else {
      if (IFrameSplitOperator.IsString(data)) result = data + ' ';
    }
  ...
  this.CON2STR = function(data, n) {
    ...
    if (Array.isArray(data)) {
      ...
      if (this.IsNumber(data)) result = data.toFixed(n);
    }
  ...
  this.VAR2STR = function(data, n) {
    ...
    if (Array.isArray(data)) {
      for (var i = 0; i < data.length; ++i) {
        result[i] = null;
        var item = data[i];
        if (this.IsNumber(item)) result[i] = item.toFixed(n);
      }
    } else {
      if (this.IsNumber(data)) result = data.toFixed(n);
    }
  ...
  this.ROUND = function(data) {
    ...
    if (Array.isArray(data)) {
      for (var i in data) {
        var item = data[i];
        if (this.IsNumber(item)) result[i] = Math.round(item);
        else result[i] = null;
      }
    }
  ...
  this.ROUND2 = function(data, n) {
    var ROUND2_SEED = [1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000, 10000000000, 100000000000, 1000000000000];
    var decimal = 0;
...
JSAlgorithm.MOD = function(number, divisor) {
  if ((number < 0 && divisor < 0) || (number >= 0 && divisor >= 0)) {
    //同号
    if (parseInt(number) == number && parseInt(divisor) == divisor) {
      //全为整数
...
function JSDraw(errorHandler, symbolData) {
  ...
  this.STICKLINE = function(condition, data, data2, width, type) {
    ...
    if (Array.isArray(condition)) {
      ...
      for (
        var i = 0;
        i < this.SymbolData.Data.Data.length;
        ++i //以K线长度为数据长度
      ) {
        drawData[i] = null;
  ...
  this.DRAWICON = function(condition, data, type) {
    if (IFrameSplitOperator.IsString(type)) {
      //把ICO1=>1
    ...
    for (var i in data) {
      ...
      for (var j in condition) {
        ...
        if (Array.isArray(item.Cond)) {
          ...
          if (this.IsNumber(item.Cond) && item.Cond) {
            //单数值条件
            rgb = item.RGB;
            break;
          }
  ...
  this.DRAWRECTREL = function(left, top, right, bottom, color) {
    let drawData = {
      Rect: { Left: Math.min(left, right), Top: Math.min(top, bottom), Right: Math.max(right, left), Bottom: Math.max(bottom, top) },
      Color: color,
    };
  ...
  this.DRAWTEXTREL = function(x, y, text) {
    ...
    if (IFrameSplitOperator.IsString(text)) drawData.Text = text;
    else if (IFrameSplitOperator.IsNonEmptyArray(text)) drawData.Text = text[0];
...
function JSSymbolData(ast, option, jsExecute) {
  ...
  this.KLineApiUrl = g_JSComplierResource.Domain + '/API/KLine2eric'; //日线
  ...
  if (option) {
    ...
    if (option.Data) {
      ...
      if (
        this.DataType != HQ_DATA_TYPE.MINUTE_ID &&
        this.DataType != HQ_DATA_TYPE.MULTIDAY_MINUTE_ID &&
        this.DataType != HQ_DATA_TYPE.HISTORY_MINUTE_ID
      ) {
        //分钟走势图数据 没有周期和复权
  ...
  this.RecvIndexHistroyData = function(data) {
    ...
    if (this.IsApiPeriod == true) {
      ...
      if (ChartData.IsDayPeriod(this.Period, false)) {
        //周期数据
  ...
  this.RecvIndexMinuteHistroyData = function(data) {
    ...
    if (this.IsApiPeriod == true) {
      ...
      if (ChartData.IsMinutePeriod(this.Period, false)) {
        //周期数据
        let periodData = this.IndexData.GetPeriodData(this.Period);
        this.IndexData.Data = periodData;
      }
  ...
  this.GetOtherSymbolParam = function(name) {
    ...
    var symbol = args[0];
    if (symbol.length == 6) {
      if (symbol[0] == '6' || symbol[0] == '5' || symbol[0] == '8' || symbol[0] == '9') symbol += '.SH';
      else if (symbol[0] == '0' || symbol[0] == '1' || symbol[0] == '2' || symbol[0] == '3') symbol += '.SZ';
  ...
  this.GetOtherSymbolData = function(job) {
    ...
    if (job.Literal) {
      ...
      if (args.length > 0) {
        ...
        if (item.Type == Syntax.Literal) {
          symbol = item.Value;
        } else if (item.Type == Syntax.Identifier) {
          //变量 !!只支持默认的变量值
          var isFind = false;
    ...
    var self = this;
    if (this.DataType == HQ_DATA_TYPE.KLINE_ID && ChartData.IsDayPeriod(this.Period, true)) {
      //请求日线数据
      if (this.NetworkFilter) {
        var dateRange = this.Data.GetDateRange();
        var obj = {
      ...
      ChartData.IsMinutePeriod(this.Period, true) ||
      this.DataType == HQ_DATA_TYPE.MINUTE_ID ||
      this.DataType == HQ_DATA_TYPE.MULTIDAY_MINUTE_ID
    ) {
      //请求分钟数据
  ...
  this.RecvOtherSymbolKData = function(data, job) {
    ...
    if (this.DataType == HQ_DATA_TYPE.KLINE_ID && ChartData.IsDayPeriod(this.Period, true)) {
      ...
      kData.DataType = 1;
      //走势图使用1分钟K线模式
      if (this.DataType == HQ_DATA_TYPE.MINUTE_ID || this.DataType == HQ_DATA_TYPE.MULTIDAY_MINUTE_ID) period = 4;
    }
  ...
  this.GetOtherSymolCacheData = function(obj) {
    ...
    switch (dataName) {
      ...
      case 'VOL':
      case 'V':
        if (MARKET_SUFFIX_NAME.IsSHSZ(upperSymbol)) return kData.GetVol(100); //A股的 把股转成手
        return kData.GetVol();
      case 'OPEN':
      case 'O':
  ...
  this.GetBlockSymbol = function(symbol) {
    //中文对应板块代码
    const BLOCK_CN_NAME_MAP = new Map([
      ['沪深A股', 'CNA.ci'],
      ['创业板', 'GEM.ci'],
      ['沪市A股', 'SHA.ci'],
      ['中小板', 'SME.ci'],
    ...
    if (upperSymbol.indexOf('.SH') > 0 || upperSymbol.indexOf('.SZ') > 0) {
      const INDEX_SYMBOL_SET = new Set([
        '000001.SH',
        '000003.SH',
        '000016.SH',
        '000300.SH',
  ...
  this.GetIndexIncreaseData = function(job) {
    ...
    if (this.NetworkFilter) {
      ...
      var obj = {
        ...
        DateRange: dateRange,
        DataType: dataType,
        Request: { Url: 'www.121287.com', Type: 'POST', Data: { symbol: this.Symbol, blocksymbol: blockSymbol, field: ['up', 'down'] } },
        Self: this,
        PreventDefault: false,
      };
      this.NetworkFilter(obj, function(data) {
    ...
    } else if (this.DataType === HQ_DATA_TYPE.KLINE_ID && ChartData.IsDayPeriod(this.Period, true)) {
      //K线图 日线
  ...
  this.GetIndexIncreaseCacheData = function(funcName, symbol, node) {
    ...
    var key;
    if (funcName == 'UPCOUNT' || funcName == 'ADVANCE')
      key = JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_INDEX_INCREASE_DATA.toString() + '-UpCount-' + blockSymbol;
    else if (funcName == 'DOWNCOUNT' || funcName == 'DECLINE')
      key = JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_INDEX_INCREASE_DATA.toString() + '-DownCount-' + blockSymbol;
    ...
    if (ChartData.IsDayPeriod(this.Period, true)) {
      ...
      if (this.NetworkFilter) {
        ...
        if (this.KLineDateTimeRange) {
          obj.Request.KLineDataTimeRange = {
            Start: { Date: this.KLineDateTimeRange.Start.Date },
            End: { Date: this.KLineDateTimeRange.End.Date },
          };
          if (this.IsNumber(this.KLineDateTimeRange.Start.Time))
            obj.Request.KLineDataTimeRange.Start.Time = this.KLineDateTimeRange.Start.Time;
  ...
  this.GetSymbolCacheData = function(dataName) {
    ...
    switch (dataName) {
      ...
      case 'VOL':
      case 'V':
        if (MARKET_SUFFIX_NAME.IsSHSZ(upperSymbol)) return this.Data.GetVol(100); //A股的 把股转成手
  ...
  this.GetSymbolPeriodCacheData = function(valueName, periodName) {
    ...
    if (this.Period == periodInfo.Period) return this.GetSymbolCacheData(valueName);
    ...
    if (this.IsApiPeriod) {
      ...
      if (ChartData.IsDayPeriod(periodInfo.Period, true) && ChartData.IsMinutePeriod(this.Period, true)) {
        ...
        if (ChartData.IsDayPeriod(periodInfo.Period, true) && bindData.Right > 0) {
          //日线数据才复权
    ...
    switch (valueName) {
      ...
      case 'V':
      case 'VOL':
        if (MARKET_SUFFIX_NAME.IsSHSZ(upperSymbol)) return result.GetVol(100);
  ...
  this.GetCurrBarsCount = function() {
    ...
    for (let i = lCount - 1; i >= 0; --i) result.push(i + 1); //数据从0开始
  ...
  this.GetBarPos = function() {
    ...
    let lCount = this.Data.Data.length;
    for (let i = 0; i < lCount; ++i) result.push(i + 1);
  ...
  this.RecvMarginData = function(data, jobID) {
    ...
    for (let i in stock.stockday) {
      ...
      switch (jobID) {
        ...
        case JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_BUY_NET: //买入信息-融资净买入
          ...
          if (!buyData) continue;
          if (
            !this.IsNumber(buyData.balance) ||
            !this.IsNumber(buyData.amount) ||
            !this.IsNumber(buyData.repay) ||
            !this.IsNumber(buyData.net)
          )
            continue;
        ...
        case JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_SELL_NET: //卖出信息-融券净卖出
          ...
          if (
            !this.IsNumber(sellData.balance) ||
            !this.IsNumber(sellData.volume) ||
            !this.IsNumber(sellData.repay) ||
            !this.IsNumber(sellData.net)
          )
            continue;
    ...
    if (jobID === JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_BALANCE || jobID === JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_RATE) {
      ...
      jobID === JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_BUY_BALANCE ||
      jobID === JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_BUY_AMOUNT ||
      ...
      jobID === JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_SELL_BALANCE ||
      jobID === JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_SELL_VOLUME ||
      jobID === JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_SELL_REPAY ||
      jobID === JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_SELL_NET
    ) {
  ...
  this.GetHKToSHSZData = function(job) {
    ...
    else this.GetHKToSHSZStockData(code, job); //下载个股数据
  };
  ...
  this.RecvMulitMinuteHKToSHSZData = function(
    data,
    job //多日分时图北上资金
  ) {
  ...
  this.RecvNewsAnalysisData = function(data, jobID) {
    ...
    if (
      jobID == JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_NEWS_ANALYSIS_HOLDERCHANGE ||
      jobID == JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_NEWS_ANALYSIS_HOLDERCHANGE2
    ) {
      var aryData = [],
        aryData2 = [];
      for (var i = 0; i < data.data.length; ++i) {
  ...
  this.GetSectionFinanceCacheData = function(year, quarter, fieldName, node) {
    ...
    var sfKey = period.Year + '-' + period.Quarter;
    if (!this.SectionFinanceData.has(sfKey) && !this.ThrowSFPeirod.has(sfKey)) {
      //动态下载的数据, 抛异常以后重新下载执行
    ...
    if (this.NetworkFilter) {
      ...
      var obj = {
        ...
        Request: {
          Url: 'www.121287.com',
          Type: 'POST',
          Data: { VariantName: jobItem.VariantName, symbol: this.Symbol, daterange: dateRange },
        },
        Self: this,
        PreventDefault: false,
      };
  ...
  this.RecvStockValue = function(recvData, jobItem, key, dataType) {
    ...
    if (dataType == 0) {
      if (Array.isArray(recvData)) {
        ...
        var aryFittingData;
        if (ChartData.IsDayPeriod(this.Period, true)) aryFittingData = kdata.GetFittingFinanceData(recvData);
        //数据和主图K线拟合
        else if (ChartData.IsMinutePeriod(this.Period, true)) aryFittingData = kdata.GetMinuteFittingFinanceData(recvData);
        //数据和主图K线拟合
        else return;
        ...
        if (key == 'EXCHANGE') {
          ...
          for (var i in result) {
            ...
            if (IFrameSplitOperator.IsPlusNumber(result[i])) result[i] = (kitem.Vol / result[i]) * 100;
          }
        }
      ...
      var kdata = this.Data; //K线
      var aryFittingData;
      if (ChartData.IsDayPeriod(this.Period, true)) aryFittingData = kdata.GetFittingTradeData(recvData, 0);
      //数据和主图K线拟合
      else if (ChartData.IsMinutePeriod(this.Period, true)) aryFittingData = kdata.GetMinuteFittingTradeData(recvData, 0);
      //数据和主图K线拟合
      else if (ChartData.IsTickPeriod(this.Period)) aryFittingData = kdata.GetMinuteFittingTradeData(recvData, 0);
      //数据和主图K线拟合
      else return;
  ...
  this.GetStockCacheData = function(obj) {
    var key;
    if (obj.FunctionName) key = this.GetStockDataKey({ FunctionName: obj.FunctionName }, obj.Args);
    else if (obj.VariantName) key = obj.VariantName;
    else if (obj.CustomName) key = obj.CustomName;
    //自定义名字
    else return null;
  ...
  this.RecvCustomAPIData = function(recvData, args) {
    ...
    if (recvData.Type == 1) this.RecvCustomAPISingleData(recvData, args);
    else this.RecvCustomAPIGroupData(recvData, args);
  };
  ...
  this.FittingCustomAPIFinanceArray = function(data, date, time) {
    ...
    var aryFittingData;
    if (ChartData.IsDayPeriod(this.Period, true)) aryFittingData = kdata.GetFittingFinanceData(arySingleData);
    else if (ChartData.IsMinutePeriod(this.Period, true)) aryFittingData = kdata.GetMinuteFittingFinanceData(arySingleData);
    else return null;
  ...
  this.FittingCustomAPIArray = function(data, date, time) {
    ...
    var aryFittingData;
    if (ChartData.IsDayPeriod(this.Period, true)) aryFittingData = kdata.GetFittingData(arySingleData);
    //数据和主图K线拟合
    else if (ChartData.IsMinutePeriod(this.Period, true)) aryFittingData = kdata.GetMinuteFittingData(arySingleData);
    //数据和主图K线拟合
    else return null;
  ...
  this.ReadArgumentValue = function(
    item,
    result //读取变量值
  ) {
  ...
  //MA.MA1#WEEK
  this.ReadIndexFunctionValue = function(
    item,
    result //返回 {Period:周期, Out:输出变量, Error:, Name:脚本名字 }
  ) {
    var indexParam = {};
  ...
  this.ReadSymbolArgumentValue = function(
    item,
    result //返回{ Value:股票代码, Error:错误信息}
  ) {
    var readArgument = {};
    if (typeof item === 'object') {
  ...
  this.CallMemberScriptIndex = function(job) {
    ...
    if (PropertyName == '' || PropertyName == null) {
      var token = job.Token;
      this.Execute.ErrorHandler.ThrowError(
        token.Index,
        token.Line,
        0,
        `CallMemberScriptIndex() Error: ${objName}.${PropertyName} 指标输出变量错误`
      );
    }
    ...
    if (!this.ReadIndexFunctionValue(callInfo, indexInfo)) {
      //读取指标
      var token = job.Token;
    ...
    if (!systemItem) {
      ...
      this.Execute.ErrorHandler.ThrowError(
        ...
        token.Line,
        0,
        `CallMemberScriptIndex() Error: '${callInfo}' ${indexInfo.Name} 指标不存在`
      );
    }
    ...
    //执行脚本
    var run = JSComplier.Execute(systemItem.Script, option, (error, indexInfo) => {
      this.ExecuteScriptIndexError(error, indexInfo);
    });
  };
  ...
  this.CallScriptIndex = function(job) {
    ...
    if (!this.ReadSymbolArgumentValue(job.Args[0], indexInfo)) {
      //读取代码
      var token = job.Token;
      this.Execute.ErrorHandler.ThrowError(token.Index, token.Line, 0, `CallScriptIndex() Error: ${indexInfo.Error}`);
    }

    if (!this.ReadIndexFunctionValue(job.Args[1], indexInfo)) {
      //读取指标
      var token = job.Token;
      this.Execute.ErrorHandler.ThrowError(token.Index, token.Line, 0, `CallScriptIndex() Error: ${indexInfo.Error}`);
    }
    ...
    if (!this.ReadIndexArgumentValue(job.Args, indexInfo)) {
      ...
      this.Execute.ErrorHandler.ThrowError(
        ...
        token.Line,
        0,
        `CallScriptIndex() ${indexInfo.Name} 指标参数错误 : ${indexInfo.Error} `
      );
    }
    ...
    //执行脚本
    var run = JSComplier.Execute(indexInfo.SytemIndex.Script, option, (error, indexInfo) => {
      this.ExecuteScriptIndexError(error, indexInfo);
    });
  };
  ...
  this.GetScriptIndexOutData = function(args, node) {
    ...
    if (!this.ReadSymbolArgumentValue(args[0], indexInfo))
      //读取代码
      this.Execute.ThrowUnexpectedNode(node, `STKINDI() 股票代码错误: ${indexInfo.Error}`);

    if (!this.ReadIndexFunctionValue(args[1], indexInfo))
      //读取指标
      this.Execute.ThrowUnexpectedNode(node, `STKINDI() 指标错误: ${indexInfo.Error}`);

    var systemIndex = new JSIndexScript();
    var systemItem = systemIndex.Get(indexInfo.Name);
    if (!systemItem) this.Execute.ThrowUnexpectedNode(node, `STKINDI() 指标错误: ${indexInfo.Name} 指标不存在`);
  ...
  this.JsonDataToMinuteData = function(data) {
    ...
    for (var i in data.stock[0].minute) {
      ...
      else item.DateTime = data.stock[0].date.toString() + ' ' + jsData.time.toString();
  ...
  this.YEAR = function() {
    ...
    for (let i in this.Data.Data) {
      var item = this.Data.Data[i];
      if (this.IsNumber(item.Date)) result[i] = parseInt(item.Date / 10000);
      else result[i] = null;
    }
  ...
  this.DAY = function() {
    ...
    for (let i in this.Data.Data) {
      var item = this.Data.Data[i];
      if (this.IsNumber(item.Date)) result[i] = parseInt(item.Date % 100);
      else result[i] = null;
    }
  ...
  this.MONTH = function() {
    ...
    for (let i in this.Data.Data) {
      var item = this.Data.Data[i];
      if (this.IsNumber(item.Date)) result[i] = parseInt((item.Date % 10000) / 100);
      else result[i] = null;
    }
  ...
  this.TIME = function() {
    ...
    for (let i in this.Data.Data) {
      var item = this.Data.Data[i];
      if (this.IsNumber(item.Time)) result[i] = item.Time;
      else result[i] = 0;
    }
  ...
  this.PERIOD = function() {
    ...
    if (this.Period >= 0 && this.Period <= PERIOD_MAP.length - 1) return PERIOD_MAP[this.Period];
  ...
  this.SysParam = function(id, jsExec) {
    ...
    if (id == 2) {
      ...
      if (IFrameSplitOperator.IsNumber(this.DrawInfo.Start)) return this.DrawInfo.Start + 1;
    } else if (id == 3) {
      jsExec.IsUsePageData = true;
      if (IFrameSplitOperator.IsNumber(this.DrawInfo.End)) return this.DrawInfo.End + 1;
    } else if (id == 4) {
      jsExec.IsUsePageData = true;
...
var JS_EXECUTE_JOB_ID = {
  ...
  GetSectionReportPeriod: function(year, quarter) {
    if (year >= 2000 && quarter >= 1 && quarter <= 4) return { Year: year, Quarter: quarter };
    return null;
  },
};
...
function JSExecute(ast, option) {
  ...
  this.ConstVarTable = new Map([
    ...
    ['ISEQUAL', null],
    ['ISUP', null],
    ['ISDOWN'], //ISUP=收阳 ISEQUAL=平盘 ISDOWN=收阴
  ...
  if (option) {
    ...
    if (option.CallbackParam) {
      ...
      if (this.CallbackParam.HQChart)
        this.GetEventCallback = id => {
          return this.CallbackParam.HQChart.GetEventCallback(id);
        };
    }
    if (option.Arguments) this.Arguments = option.Arguments;
  ...
  this.ReadMemberVariable = function(node) {
    ...
    let maiObj;
    if (obj.Type == Syntax.BinaryExpression || obj.Type == Syntax.LogicalExpression) maiObj = this.VisitBinaryExpression(obj);
    else if (obj.Type == Syntax.CallExpression) maiObj = this.VisitCallExpression(obj);
    else maiObj = this.GetNodeValue(obj);
    ...
    for (let i = 0; i < this.AST.Body.length; ++i) {
      ...
      if (item.Type == Syntax.ExpressionStatement && item.Expression) {
        if (item.Expression.Type == Syntax.AssignmentExpression) {
          ...
          this.OutVarTable.push({ Name: varName, Data: outVar, Type: type, NoneName: true });
        } else if (item.Expression.Type == Syntax.Literal) {
          //常量
          ...
          if (!Array.isArray(outVar)) outVar = this.SingleDataToArrayData(outVar);
          this.OutVarTable.push({ Name: varName, Data: outVar, Type: type, NoneName: true });
        } else if (item.Expression.Type == Syntax.LogicalExpression) {
          //逻辑语句 如 T1 AND T2
          ...
          for (let j = 0; j < item.Expression.Expression.length; ++j) {
            ...
            if (itemExpression.Type == Syntax.AssignmentExpression && itemExpression.Operator == ':' && itemExpression.Left) {
              ...
              else if (value.indexOf('EXDATA') == 0) isExData = true;
              //扩展数据, 不显示再图形里面
              else if (value.indexOf('LINEOVERLAY') == 0) isOverlayLine = true;
              else if (value.indexOf('NOTEXT') == 0 || value.indexOf('NOTITLE') == 0) isShowTitle = false;
              //标题不显示
          ...
          if (pointDot && varName) {
            ...
            if (lineWidth) value.LineWidth = lineWidth;
            this.OutVarTable.push(value);
          } else if (volStick && varName) {
            //VOLSTICK   画彩色柱状线
            let outVar = this.VarTable.get(varName);
            let value = { Name: varName, Data: outVar, Type: 6 };
            if (color) value.Color = color;
            this.OutVarTable.push(value);
          } else if (colorStick && varName) {
    ...
    try {
      ...
      if (this.UpdateUICallback) {
        ...
        if (this.CallbackParam && this.CallbackParam.Job && this.CallbackParam.Job.ID == JS_EXECUTE_JOB_ID.JOB_EXECUTE_INDEX) {
          ...
          if (this.CallbackParam && this.CallbackParam.Self && this.CallbackParam.Self.ClassName === 'ScriptIndexConsole')
            this.CallbackParam.JSExecute = this;
  ...
  this.VisitCallExpression = function(node) {
    ...
    for (let i in node.Arguments) {
      ...
      let value;
      if (item.Type == Syntax.BinaryExpression || item.Type == Syntax.LogicalExpression) value = this.VisitBinaryExpression(item);
      else if (item.Type == Syntax.CallExpression) value = this.VisitCallExpression(item);
      else value = this.GetNodeValue(item);
      args.push(value);
    }
    ...
    switch (funcName) {
      ...
      case 'COVER_V':
        if (args.length == 2)
          return this.SymbolData.GetSymbolPeriodCacheData2(JSComplierHelper.GetConvertValueName(funcName), args[0], args[1]);
  ...
  this.VisitAssignmentExpression = function(node) {
    ...
    let value = null;
    if (right.Type == Syntax.BinaryExpression || right.Type == Syntax.LogicalExpression) value = this.VisitBinaryExpression(right);
    else if (right.Type == Syntax.CallExpression) value = this.VisitCallExpression(right);
    ...
    else if (right.Type == Syntax.MemberExpression) value = this.ReadMemberVariable(right);
  ...
  this.VisitBinaryExpression = function(node) {
    ...
    while (stack.length != 0) {
      ...
      if (temp.Left && node != temp.Left && node != temp.Right) {
        ...
        let value = stack.pop();
        if (value.Type == Syntax.BinaryExpression) {
          //只遍历操作符就可以
          let leftValue = this.GetNodeValue(value.Left);
          let rightValue = this.GetNodeValue(value.Right);

          if (JS_EXECUTE_DEBUG_LOG)
            JSConsole.Complier.Log('[JSExecute::VisitBinaryExpression] BinaryExpression', value, leftValue, rightValue);
...
function JSExplainer(ast, option) {
  ...
  this.FUNCTION_INFO_LIST = new Map([
    ...
    [
      'STRCMP',
      {
        ...
        Param: { Count: 2 },
        ToString: function(args) {
          return `字符串${args[0]}和字符串${args[1]}比较`;
        },
      },
    ],
    [
  ...
  this.CallFunctionExplain = function(funcName, args, node) {
    if (this.FUNCTION_INFO_LIST.has(funcName)) {
      ...
      if (item.Param.Count != args.length) this.ThrowUnexpectedNode(node, `函数${funcName}参数个数不正确. 需要${item.Param.Count}个参数`);
  ...
  this.VisitAssignmentExpression = function(node) {
    ...
    if (right.Type == Syntax.BinaryExpression || right.Type == Syntax.LogicalExpression) value = this.VisitBinaryExpression(right);
      ...
      value = right.Value;
      if (IFrameSplitOperator.IsString(value) && right.Value.indexOf('$') > 0) value = this.GetOtherSymbolExplain({ Literal: value }, node);
    } else if (right.Type == Syntax.Identifier)
      //右值是变量
      value = this.ReadVariable(right.Name, right);
    else if (right.Type == Syntax.MemberExpression) value = this.ReadMemberVariable(right);
...
JSComplier.AddIcon = function(
  obj //添加一个obj={ID:, Text:, Color, Family: }
) {
  g_JSComplierResource.CustomDrawIcon.Data.set(obj.ID, obj);
};

JSComplier.AddFunction = function(
  obj //添加函数 { Name:函数名, Description:描述信息, IsDownload:是否需要下载数据, Invoke:函数执行(可选) }
) {
...
JSComplier.AddVariant = function(
  obj //{ Name:变量名, Description:描述信息 }
) {
  if (!obj || !obj.Name) return;
...
function ScriptIndex(name, script, args, option) {
  ...
  this.ExecuteScript = function(hqChart, windowIndex, hisData) {
    ...
    if (hqChart.ClassName === 'MinuteChartContainer' || hqChart.ClassName === 'MinuteChartHScreenContainer') {
      if (hqChart.DayCount > 1) hqDataType = HQ_DATA_TYPE.MULTIDAY_MINUTE_ID;
      //多日分钟
  ...
  this.RecvResultData = function(outVar, param) {
    ...
    if (param.Self.IsLocked == false) {
      ...
      let lockData = {
        IsLocked: true,
        Callback: param.Self.LockCallback,
        IndexName: param.Self.Name,
        ID: param.Self.LockID,
        BG: param.Self.LockBG,
        Text: param.Self.LockText,
        TextColor: param.Self.LockTextColor,
        Font: param.Self.LockFont,
        Count: param.Self.LockCount,
        MinWidth: param.Self.LockMinWidth,
      };
  ...
  this.CreateLine = function(hqChart, windowIndex, varItem, id) {
    ...
    if (varItem.IsShowTitle === false) {
      //NOTEXT 不绘制标题
    } else if (IFrameSplitOperator.IsString(varItem.Name) && varItem.Name.indexOf('NOTEXT') == 0) {
      //标题中包含NOTEXT不绘制标题
    } else {
      if (varItem.NoneName) hqChart.TitlePaint[titleIndex].Data[id] = new DynamicTitleData(line.Data, null, line.Color);
      else hqChart.TitlePaint[titleIndex].Data[id] = new DynamicTitleData(line.Data, varItem.Name, line.Color);
    }
  ...
  this.CreateStraightLine = function(hqChart, windowIndex, varItem, id) {
    ...
    if (varItem.Name == 'DRAWLINE') hqChart.TitlePaint[titleIndex].Data[id] = new DynamicTitleData(line.Data, null, line.Color);
  ...
  this.BindInstructionData = function(
    hqChart,
    windowIndex,
    hisData //绑定指示指标
  ) {
  ...
  this.BindData = function(hqChart, windowIndex, hisData) {
    ...
    for (let i in this.OutVar) {
      ...
      if (titlePaint && titlePaint.Data && i < titlePaint.Data.length) {
        //设置标题数值 小数位数和格式
...
function APIScriptIndex(name, script, args, option, isOverlay) {
  ...
  this.ExecuteScript = function(hqChart, windowIndex, hisData) {
    ...
    if (hqChart.NetworkFilter) {
      ...
      hqChart.NetworkFilter(obj, function(data) {
        if (self.Version == 2) self.RecvAPIData2(data, hqChart, windowIndex, hisData);
        else self.RecvAPIData(data, hqChart, windowIndex, hisData);
      });
  ...
  this.ConvertToLocalData = function(jsonData, hqChart) {
    ...
    if (hqChart.ClassName == 'MinuteChartContainer' || hqChart.ClassName == 'MinuteChartHScreenContainer') {
      ...
      if (ChartData.IsDayPeriod(jsonData.Period, true)) var aryDataIndex = kdata.GetAPIDataIndex(jsonData.Date);
      else var aryDataIndex = kdata.GetAPIDataIndex(jsonData.Date, jsonData.Time);
    }
    ...
    for (var i in outVar) {
      ...
      if (item.Type == 0) {
        ...
        if (outItem.Name && outItem.Name.indexOf('@NODRAW@') >= 0) outItem.Name = outItem.Name.replace('@NODRAW@', '');
      } else if (item.Type == 1) {
        //绘图函数
          ...
          case 'DRAWKLINE':
            {
              ...
              for (var j = 0; j < aryDataIndex.length; ++j) {
                ...
                if (
                  IFrameSplitOperator.IsPlusNumber(high) &&
                  IFrameSplitOperator.IsPlusNumber(low) &&
                  IFrameSplitOperator.IsPlusNumber(open) &&
                  IFrameSplitOperator.IsPlusNumber(close)
                ) {
                  item.Open = open;
                  item.Close = close;
                  item.High = high;
  ...
  this.RecvAPIData = function(data, hqChart, windowIndex, hisData) {
    ...
    if (this.IsLocked == false) {
      ...
      let lockData = {
        ...
        BG: this.LockBG,
        Text: this.LockText,
        TextColor: this.LockTextColor,
        Font: this.LockFont,
        Count: this.LockCount,
        MinWidth: this.LockMinWidth,
      };
  ...
  this.FittingArray = function(
    sourceData,
    date,
    time,
    hqChart,
    arrayType //arrayType 0=单值数组 1=结构体
  ) {
    ...
    var aryFittingData;
    if (ChartData.IsDayPeriod(hqChart.Period, true)) aryFittingData = kdata.GetFittingData(arySingleData);
    //数据和主图K线拟合
    else aryFittingData = kdata.GetMinuteFittingData(arySingleData); //数据和主图K线拟合
  ...
  this.FittingMultiLine = function(sourceData, date, time, hqChart) {
    ...
    if (ChartData.IsDayPeriod(hqChart.Period, true)) {
      ...
      return sourceData;
    } else if (ChartData.IsMinutePeriod(hqChart.Period, true) || ChartData.IsTickPeriod(hqChart.Period)) {
      //分钟线
      var kdata = hqChart.ChartPaint[0].Data; //K线
    ...
    for (var i in outVar) {
      ...
      if (item.data) {
        ...
        if (draw.DrawType == 'DRAWICON') {
          ...
          result.push(outVarItem);
        } else if (draw.DrawType == 'STICKLINE') {
          //柱子
          drawItem.Name = draw.Name;
          ...
          result.push(outVarItem);
        } else if (draw.DrawType == 'MULTI_HTMLDOM') {
          //外部自己创建dom
          drawItem.Text = draw.Text;
          drawItem.Name = draw.Name;
          drawItem.DrawType = draw.DrawType;
  ...
  this.GetKLineData = function(data, hqChart) {
    ...
    if (this.HQDataType == HQ_DATA_TYPE.MINUTE_ID || this.HQDataType == HQ_DATA_TYPE.MULTIDAY_MINUTE_ID) var kData = hqChart.SourceData;
    //走势图分钟数据
  ...
  this.FittingKLineBG = function(data, hqChart) {
    ...
    } else if (ChartData.IsMinutePeriod(hqChart.Period, true)) {
      //分钟线
      var bFill = false;
      for (var i = 0, j = 0; i < kData.Data.length; ) {
        result[i] = 0;
  ...
  this.FittingKLineBG_V2 = function(
    data,
    hqChart //2.0版本通过  [{ Start:, End: }, .....]来填充背景色
  ) {
    var kData = hqChart.ChartPaint[0].Data; //K线
    var result = [];
    if (ChartData.IsDayPeriod(hqChart.Period, true)) {
    ...
    if (ChartData.IsDayPeriod(period, true)) {
      ...
      if (firstItem.Date > firstKItem.Date) {
        ...
        for (kIndex = 0; kIndex < kdata.Data.length; ++kIndex) {
          ...
          if (kItem.Date == firstItem.Date) break;
        ...
        for (dataIndex = 0; dataIndex < arySrouceData.length; ++dataIndex) {
          ...
          if (item.Date >= firstKItem.Date) break;
        }
      }
...
function LocalJsonDataIndex(name, args, option) {
  ...
  this.FittingData = function(jsonData, hisData) {
    ...
    for (i in outVar) {
      ...
      if (hisData.Period < 4) var aryFittingData = hisData.GetFittingData(indexData);
      //数据和主图K线拟合
...
function ScriptIndexConsole(obj) {
  ...
  this.RecvResultData = function(outVar, param) {
    ...
    var date = null;
    if (jsExec.SymbolData.Data) date = jsExec.SymbolData.Data.GetDate();
    ...
    if (jsExec.SymbolData.DataType == HQ_DATA_TYPE.MULTIDAY_MINUTE_ID || jsExec.SymbolData.DataType == HQ_DATA_TYPE.MINUTE_ID)
      result.Time = jsExec.SymbolData.Data.GetTime();
    else if (jsExec.SymbolData.DataType == HQ_DATA_TYPE.KLINE_ID && ChartData.IsMinutePeriod(jsExec.SymbolData.Period, true))
      result.Time = jsExec.SymbolData.Data.GetTime();
...
ScriptIndexConsole.SetDomain = function(
  domain,
  cacheDomain //修改API地址
) {
...
function DownloadFinOneData(obj) {
  ...
  this.Download = function() {
    ...
    if (year == 0 && month == 0) {
      //如果Y和MMDD都为0,表示最新的财报;
    } else if (year == 0 && month < 300) {
      //如果Y为0,MMDD为小于300的数字,表示最近一期向前推MMDD期的数据,如果是331,630,930,1231这些,表示最近一期的对应季报的数据;
      dataIndex = month;
      dataEnd = 200;
    } else if (month == 0 && year < 1000) {
      //如果Y为0,MMDD为小于300的数字,表示最近一期向前推MMDD期的数据,如果是331,630,930,1231这些,表示最近一期的对应季报的数据;
      preYear = year;
    } else if (year > 1909) {
      if (month == 331) {
        aryCondition = [{ item: ['announcement1.year', 'int32', 'eq', year] }, { item: ['finance1', 'doc', 'exists', 'true'] }];

        fieldList.push('announcement1.year');
        fieldList.push('announcement1.quarter');
      } else if (month == 630) {
        aryCondition = [{ item: ['announcement2.year', 'int32', 'eq', year] }, { item: ['finance2', 'doc', 'exists', 'true'] }];

        fieldList.push('announcement2.year');
        fieldList.push('announcement2.quarter');
      } else if (month == 930) {
        aryCondition = [{ item: ['announcement3.year', 'int32', 'eq', year] }, { item: ['finance3', 'doc', 'exists', 'true'] }];

        fieldList.push('announcement4.year');
        fieldList.push('announcement4.quarter');
      } else {
        aryCondition = [{ item: ['announcement4.year', 'int32', 'eq', year] }, { item: ['finance4', 'doc', 'exists', 'true'] }];
    ...
    JSNetwork.HttpRequest({
      ...
      success: function(recvData) {
        ...
        if (data && data.length > 0) {
          ...
          if (preYear == null) result = data[dataIndex];
          //返回一个数据
          else result = self.GetPreYearData(data, preYear);
        }
  ...
  this.ToHQChartItemData = function(item, id, subID) {
    ...
    switch (id) {
      ...
      case 2:
        ...
        if (subID == 0) return { Date: date, Value: item.tradedetail[0].buy };
        else return { Date: date, Value: item.tradedetail[0].sell };
      case 3:
        if (!item.margin) return null;
        if (subID == 0) {
          if (item.margin.buy) return { Date: date, Value: item.margin.buy.balance };
        } else {
          if (item.margin.sell) return { Date: date, Value: item.margin.sell.balance };
        }
        return null;
      case 4:
        if (!item.blocktrading) return null;
        if (subID == 0) return { Date: date, Value: item.blocktrading.price };
        else return { Date: date, Value: item.blocktrading.amount };
      default:
        return null;
    }
  };
